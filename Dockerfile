...
RUN sed -i 's/^reqwest = { version = "\(\d*\.*\d*\.*\d*\)", features = \[\("gzip", "deflate", "stream", "blocking", "brotli"\)\] }/reqwest = { version = "\1", default-features = false, features = \["http2", "rustls-tls", \2\] }' Dockerfile
